...
export const ChatWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      text: "Привет! Я виртуальный ассистент. Спросите меня о навыках, опыте или проектах Комилджона! 🤖",
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    setInput("");
    setMessages((prev) => [...prev, { sender: SenderType.USER, text: userMessage }]);
    setIsLoading(true);

    const aiResponse = await generateChatResponse(userMessage);

    setMessages((prev) => [...prev, { sender: SenderType.AI, text: aiResponse }]);
    setIsLoading(false);
  };
...
export const ChatWidget: React.FC = () => {
	...
	const [messages, setMessages] = useState<ChatMessage[]>([
		{
			sender: SenderType.AI,
			text: "Привет! Я виртуальный ассистент по имени Феномен. Спросите меня о навыках, опыте или проектах Комилджона! 🤖",
		},
	]);
	const messagesEndRef = useRef<HTMLDivElement>(null);
